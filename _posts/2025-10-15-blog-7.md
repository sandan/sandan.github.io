...
Let's have a look at universal quantification applied to a disjunction. In this case, we have a hypothesis that has a quantified disjunction. Just like previous quantified propositions, we can pass a natural number to remove the quantifier:
```lean
...
2. Add a disjunct to a universally true predicate
```lean
example (h : ∀ x, P x) : ∀ x, P x ∨ Q x := by
  -- hint: introduce an arbitrary n, then use `left` with `h n`
```

3. Extract a global proposition from universally quantified implications
```lean
example (R : Prop) (h : ∀ x, P x → R) (hp : ∀ x, P x) : R := by
  -- hint: pick any natural number n (your favorite number ;), use h n : P n → R and hp n : P n
```

4. Combine a pointwise implication with a pointwise disjunction
```lean
example (h1 : ∀ x, P x → Q x) (h2 : ∀ x, P x ∨ Q x) : ∀ x, Q x := by
  -- hint: for arbitrary n, examine `h2 n` with `cases`; in the `P n` case use `h1 n`
```

5. Build a pointwise conjunction from two universal facts
```lean
