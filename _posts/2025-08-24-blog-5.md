...
theorem contradiction_example (p : Prop) (h1 : p) (h2 : ¬p) : p ∧ ¬p := by
  ...
  · exact h2
```
Lean will not complain and we will have seemingly proved a contradictory statement. Surely, `p` and `¬p` cannot both be true. Just because the Lean compiler is happy with the proof does not necessarily mean you have proven something soundly. Always check that whatever you're trying to prove makes sense.
...
The `intro` tactic is fundamental for working with implications. When you have a goal of the form `p → q`, the `intro` tactic allows you to:
...
```lean
-- The identity implication: p implies p
example (p : Prop) : p → p := by
  intro h    -- Assume p as hypothesis h
  -- Now we have: h : p, and goal: ⊢ p
  exact h    -- Since we assumed p, we can prove p
```

In the first example, we looked at double negation. We can express that example differently using implication:

```lean
example (p : Prop) : p → ¬¬p := by
  intro hp   -- Assume p as hypothesis hp
  -- Now we have: hp : p, and goal: ⊢ ¬¬p
  push_neg   -- Simplify ¬¬p to p
  -- Now goal: ⊢ p  
  exact hp   -- We have p from our assumption
```

Now, try finishing this proof:
```lean
example (p : Prop) : p → ¬¬p := by
  push_neg
  sorry
...
example (p q r : Prop) (h : p → q → r) (hp : p) (hq : q) : r := by
  apply h      -- Goal becomes: prove both p and q
...
theorem implication_transitivity (p q r : Prop) : (p → q) → (q → r) → (p → r) := by
  intro h1    -- Assume p → q
  intro h2    -- Assume q → r  
  intro hp    -- Assume p (to prove p → r)
  apply h2    -- To prove r, it suffices to prove q (since q → r)
  apply h1    -- To prove q, it suffices to prove p (since p → q)
  exact hp    -- We have p from our assumption
```

This proof demonstrates the transitivity of implication: if p implies q and q implies r, then p implies r.

### Alternative: Treating Implications as Functions

There's another way to think about implications in Lean: as functions. An implication `p → q` can be applied like a function to an argument of type `p` to produce a result of type `q`:

```lean
-- Using implications as functions
example (p q : Prop) (h : p → q) (hp : p) : q := by
  exact h hp  -- Apply function h to argument hp

-- This also works with the have tactic for intermediate results
example (p q r : Prop) (h1 : p → q) (h2 : q → r) (hp : p) : r := by
  have hq := h1 hp  -- Apply h1 to hp to get q
  have hr := h2 hq  -- Apply h2 to hq to get r
  exact hr
```

### More Complex Examples

Here are some more sophisticated examples that combine these concepts:

```lean
-- Using apply with compound statements
example (p q : Prop) (h : p → q) : ¬q → ¬p := by
  intro nq    -- Assume ¬q
  intro hp    -- To prove ¬p, assume p and derive contradiction
  apply nq    -- To get contradiction, apply ¬q (which proves q → False)
  exact h hp  -- Prove q using h and hp

-- Multiple levels of implication
example (p q r s : Prop) (h : p → q → r → s) (hp : p) : q → r → s := by
  intro hq hr  -- Introduce the remaining assumptions
  apply h      -- Apply h, which needs p, q, and r
  · exact hp   -- Prove p
  · exact hq   -- Prove q  
  · exact hr   -- Prove r

-- Sometimes apply can completely solve the goal
example (p q : Prop) (h : p → q) : p → q := by
  apply h      -- Since the goal is exactly what h provides, we're done!
```
...
```lean
theorem contrapositive_tactic (p q : Prop) (h : p → q) : ¬q → ¬p := by
  intro nq hp
  apply nq
  exact h hp
```
...
In the previous chapter, we looked at proving distributivity of conjunction and disjunction. Lean has a lemma that we could have used called `and_or_left`.
```lean 
-- An alternative proof (and much shorter)
-- TODO: revisit this after discussing iff
