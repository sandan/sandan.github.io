...
```lean
/-- Use tactic with type inference -/
example : ∃ (x : ℕ × ℕ), x.1 + x.2 = 5 := by
  use (2, 3)     -- Let x be the pair of natural numbers (2, 3)
  -- ⊢ (2, 3).1 + (2, 3).2 = 5
  rfl
```
- inequalities
...
```
We have not worked with natural numbers in detail. We will in future posts where we use built-in lemmas in Lean's Mathlib (such as `lt_succ_self`) to prove propositions about natural numbers.
```lean
#check Nat.lt_succ_self
-- Nat.lt_succ_self (n : ℕ) : n < n.succ
```
The lemma takes a natural number n and returns a proof that it is strictly less than its successor (n + 1). This proof is exactly what is needed to close the goal in the previous example.
