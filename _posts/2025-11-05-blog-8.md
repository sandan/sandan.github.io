...
When we have an existentially quantified proposition in the goal, we have to construct a value (sometimes called a witness) and provide it to the goal. The `use` tactic does this. Here's our first example:
...
- multiple variables
```lean
/-- Multiple elements with use tactic -/
...
example (h : P 3 → Q 3) (p3 : P 3) : ∃x, Q x := by
  ...
  use 3
```
The `use` tactic provides the value to the existential quantifier, removing the quantifier and replacing the variable with a particular value.
...
example : (h : ∃ x, P x) → True := by
  ...
  trivial
```
We provide a hypothesis that is an existentially quantified statement to the `obtain` tactic. We get back a particular natural number `a` and a hypothesis `ha : P a`, eliminating the existential quantifier. 

 The tactic takes an existential statement and deconstructs it -- instantiating a witness and a proposition about it. The Existential Elimination rule is also called the Existential Instantiation rule for that reason.
...
```
Just like the universal quantifier, the names used in each quantifier are distinct. Even though we see `x` in the hypothesis of the previous example, it does not necessarily mean it is equal to the `x` in the goal. That is, the `x` in the hypothesis and the `x` in the goal refer to different things. We'll see this in the next example.
...
example (h : (∃ x, P x) ∧ (∃ x, Q x) ): ∃ x, P x ∧ Q x := by
    sorry
```
If we tried proving this, we'd see that just because each existentially quantified conjunct has its own `x` that satisfies `P` and `Q`, it may not be the case that that the same instance `x` satisfies `P` and `Q` simultaneously. 

Another way of expressing an equivalent hypothesis to the one in the previous example is `(∃ x, P x) ∧ (∃ y, Q y)`. Unless `x` and `y` are equal, we can't necessarily conclude `∃ z, P z ∧ Q z`.

Let's provide a counter-example to see this intuitively. For example, suppose `P` is the proposition that `x` is odd and `Q` is the proposition that `y` is even. The hypothesis says there are numbers that are odd and there are numbers that are even. However, the goal asks for at least one natural number that is both odd and even (there are none).
...
```
In this post, we looked at the existential quantifier. We saw how the `use` tactic can work on goals that are existentially quantified. We also saw how the `obtain` and `cases` tactics can work on hypotheses that are existentially quantified.

## Exercises

1. Prove that there exists a natural number greater than 2:
```lean
example : ∃ n : ℕ, n > 2 := by
  sorry
```
...
3. Prove the converse of the example in the Existential Disjunction section or give a counter-example.
