...
In the previous chapter, we went over Propositions and Conjunctions. Recall that Propositions are statements that are either true or false. Conjunctions are compound statements that join to or more propositions with "and" `∧`.

In this chapter, we discuss disjunctions. Disjunctions are compound statements that join two or more propositions with an "or". Symbolically if `p` and `q` are propositions, `p ∨ q` is a disjunction. For example:
- "An integer is positive, negative, or 0."
- "A natural number is prime or composite."
- "An integer is either even or odd."
- "An element x is in set A or is in set B."
Some of the more interesting propositions are disjunctions which seem like they are mutually exclusive but are actually inclusive.

Disjunctions assert that at least one of the propositions are true. `p ∨ q` asserts that `p` is true, `q` is true, or both
of the propositions are true. This kind of disjunction is called "inclusive or". 

Sometimes a disjunction asserts that one or the other proposition is true but not both. That is, `p ∨ q` but not `p ∧ q`. In this case, `p` and `q` are "mutually exclusive". We'll go over "exclusive or" in a future chapter after we learn more about negation.
