...
Often, to complete a proof, the key is to split the problem into cases. The fun part is choosing how to decompose the problem into cases. It's like origami. The problem is a piece of paper. The seams that you can fold are like choosing the cases to split on. A so that we can make the right shape (the goal).
